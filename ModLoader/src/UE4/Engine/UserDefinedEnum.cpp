...
namespace UE4 {
	TMap<FName, FText> UE4::UUserDefinedEnum::GetDispalyNames() const {
		return Read<TMap<FName, FText>>((byte*)this + MDML::SelectedGameProfile.defs.UEnum.UserDefDisplayNameMap);
	}
	FText UUserDefinedEnum::GetDisplayNameTextByIndex(int32_t InIndex) const {
		...
		if (fn == 0) {
			...
			Log::Info_MDML("UUserDefinedEnum::GetDisplayNameTextByIndex found at 0x{0:x}", fn);
		}
