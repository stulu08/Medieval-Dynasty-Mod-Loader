...
bool Dumper::DumpEngineInfo()
{
	...
	if (Log != NULL) {
		auto defs = MDML::SelectedGameProfile.defs;
		fprintf(Log, "#Engine Info Dump\n");
		fprintf(Log, "[GInfo]\nIsGInfoPatterns=0\nGName=0x%p\nGObject=0x%p\nGWorld=0x%p\n", MDML::SelectedGameProfile.GName - (DWORD64)GetModuleHandleW(0), MDML::SelectedGameProfile.GObject - (DWORD64)GetModuleHandleW(0), MDML::SelectedGameProfile.GWorld - (DWORD64)GetModuleHandleW(0));
		...
		fprintf(Log, "\n[UFieldDef]\nNext=0x%p\n", defs.UField.Next);
		fprintf(Log, "\n[Property]\nIsFProperty=0x%d\nArrayDim=0x%p\nElementSize=0x%p\nFlags=0x%p\nOffset=0x%p\n", MDML::SelectedGameProfile.bIsFProperty, defs.Property.ArrayDim, defs.Property.ElementSize, defs.Property.Flags, defs.Property.Offset);
