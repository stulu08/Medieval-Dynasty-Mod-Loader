...
void SetupProfile(std::string Path)
{
	...
	if (std::filesystem::exists(Profile))
	{
		...
		else
		{
			if (GameProfile::SelectedGameProfile.UsesFNamePool)
			{
				...
				if (FPoolPat != nullptr)
				{
					...
					Log::Info("FoundNamePool: 0x{0:x}", GameProfile::SelectedGameProfile.GName);
				}
				else
				{
			...
			else
			{
				...
				if (GNamePat != nullptr)
				{
					...
					Log::Info("GName: 0x{0:x}", GameProfile::SelectedGameProfile.GName);
				}
				else
				{
					Log::Error("GName Could Not Be Found!");
			...
			if (GObjectPat != nullptr)
			{
				...
				Log::Info("GObject: 0x{0:x}", GameProfile::SelectedGameProfile.GObject);
			}
			else
			{
			...
			if (GWorldPat != nullptr)
			{
				...
				Log::Info("GWorld: 0x{0:x}", GameProfile::SelectedGameProfile.GWorld);
			}
			else
			{
		...
		if (GameInfo.select("UObjectDef"))
		{
			...
			GameProfile::SelectedGameProfile.defs.UObject.Index = (uint16_t)StringToDWord(GameInfo.get("UObjectDef", "Index", ""));
			GameProfile::SelectedGameProfile.defs.UObject.Class = (uint16_t)StringToDWord(GameInfo.get("UObjectDef", "Class", ""));
			GameProfile::SelectedGameProfile.defs.UObject.Name = (uint16_t)StringToDWord(GameInfo.get("UObjectDef", "Name", ""));
			GameProfile::SelectedGameProfile.defs.UObject.Outer = (uint16_t)StringToDWord(GameInfo.get("UObjectDef", "Outer", ""));
		}
		...
		if (GameInfo.select("UFieldDef"))
		{
			...
			GameProfile::SelectedGameProfile.defs.UField.Next = (uint16_t)StringToDWord(GameInfo.get("UFieldDef", "Next", ""));
		}
		if (GameInfo.select("UStructDef"))
		{
			GameProfile::SelectedGameProfile.IsUStructMissing = false;
			GameInfo.select("UStructDef");
			GameProfile::SelectedGameProfile.defs.UStruct.SuperStruct = (uint16_t)StringToDWord(GameInfo.get("UStructDef", "SuperStruct", ""));
			GameProfile::SelectedGameProfile.defs.UStruct.Children = (uint16_t)StringToDWord(GameInfo.get("UStructDef", "Children", ""));
			GameProfile::SelectedGameProfile.defs.UStruct.PropertiesSize = (uint16_t)StringToDWord(GameInfo.get("UStructDef", "PropertiesSize", ""));
		}
		if (GameInfo.select("UFunctionDef"))
		{
			GameProfile::SelectedGameProfile.IsUFunctionMissing = false;
			GameInfo.select("UFunctionDef");
			GameProfile::SelectedGameProfile.defs.UFunction.FunctionFlags = (uint16_t)StringToDWord(GameInfo.get("UFunctionDef", "FunctionFlags", ""));
			GameProfile::SelectedGameProfile.defs.UFunction.NumParams = (uint16_t)StringToDWord(GameInfo.get("UFunctionDef", "NumParams", ""));
			GameProfile::SelectedGameProfile.defs.UFunction.ParamsSize = (uint16_t)StringToDWord(GameInfo.get("UFunctionDef", "ParamsSize", ""));
			GameProfile::SelectedGameProfile.defs.UFunction.ReturnValueOffset = (uint16_t)StringToDWord(GameInfo.get("UFunctionDef", "ReturnValueOffset", ""));
			GameProfile::SelectedGameProfile.defs.UFunction.Func = (uint16_t)StringToDWord(GameInfo.get("UFunctionDef", "Func", ""));
		}

		if (GameInfo.select("Property"))
		{
			GameProfile::SelectedGameProfile.bIsFProperty = GameInfo.getAs<int>("Property", "IsFProperty", 0);
			GameProfile::SelectedGameProfile.defs.Property.ArrayDim = (uint16_t)StringToDWord(GameInfo.get("Property", "ArrayDim", ""));
			GameProfile::SelectedGameProfile.defs.Property.Offset = (uint16_t)StringToDWord(GameInfo.get("Property", "Offset", ""));
			if (GameProfile::SelectedGameProfile.bIsFProperty)
			{
				GameInfo.select("FField");
				GameProfile::SelectedGameProfile.defs.FField.Name = (uint16_t)StringToDWord(GameInfo.get("FField", "Name", ""));
				GameProfile::SelectedGameProfile.defs.FField.Next = (uint16_t)StringToDWord(GameInfo.get("FField", "Next", ""));
			}
		}
		else
		{
		...
		else
		{
			...
			Log::Info("GameStateInit: 0x{0:x}", GameProfile::SelectedGameProfile.GameStateInit);
			...
			if (BeginPlay != nullptr)
			{
				...
				Log::Info("AActor::BeginPlay: 0x{0:x}", GameProfile::SelectedGameProfile.BeginPlay);
			}
			else
			{
			...
			Log::Info("StaticLoadObject: 0x{0:x}", (void*)GameProfile::SelectedGameProfile.StaticLoadObject);
			...
			Log::Info("UWorld::SpawnActor: 0x{0:x}", (void*)GameProfile::SelectedGameProfile.SpawnActorFTrans);
			...
			Log::Info("CallFunctionByNameWithArguments: 0x{0:x}", (void*)GameProfile::SelectedGameProfile.CallFunctionByNameWithArguments);
			...
			if (ProcessEvent != nullptr)
			{
				...
				Log::Info("UObject::ProcessEvent: 0x{0:x}", (void*)GameProfile::SelectedGameProfile.ProcessEvent);
			}
			else
			{
			...
			Log::Info("UClass::CreateDefualtObject: 0x{0:x}", (void*)GameProfile::SelectedGameProfile.CreateDefaultObject);
		}
		if (GameInfo.select("ProcessInternalFunction"))
		{
			...
			Log::Info("ProcessInternalFunction: 0x{0:x}", GameProfile::SelectedGameProfile.ProcessInternals);
		}
		else
		{
			if (GameProfile::SelectedGameProfile.UsesFNamePool || GameProfile::SelectedGameProfile.IsUsing4_22)
			{
				...
				if (ProcessAddy)
				{
					...
					Log::Info("ProcessInternalFunction: 0x{0:x}", (void*)GameProfile::SelectedGameProfile.ProcessInternals);
				}
			}
		}
		...
		else
		{
			...
			Log::Info("StaticConstructObject_Internal 0x{0:x}", GameProfile::SelectedGameProfile.StaticConstructObject_Internal);
		}
